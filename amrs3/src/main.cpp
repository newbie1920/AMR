...
// I2C: MPU6050 + OLED (không dùng GPIO 21/22 vì S3 không có mặc định)
#define IMU_SDA         41
#define IMU_SCL         42
...
void lidar_broadcast() {
  ...
  if (millis() - lastScanBroadcast < 333) return; // ~3Hz
    ...
    [](uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
      ...
      if (doc["type"] == "config") {
        ...
        if (!doc["comp_alpha"].isNull())
          COMP_FILTER_ALPHA = constrain(doc["comp_alpha"].as<float>(), 0.0f, 1.0f);
      }
  ...
  if (deltaT >= 0.02f) {
    ...
    if (millis() - lastTelemetryTime > 200) {
      ...
      if (millis() - lastOledUpdateTime > 1000) {
        ...
        display.println((fabs(targetLeftVel) > 0.01f || fabs(targetRightVel) > 0.01f)
                        ? "MOVING" : "READY");
        display.display();
      }

      TelnetStream.printf(
        "L:%ld R:%ld | vL:%.1f vR:%.1f | θ:%.1f° LiDAR:%s\n",
