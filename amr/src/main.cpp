...
long tickRounding = 0;           // Snap-to-grid rounding (0 = disabled)
...
unsigned long lastStopSettlingTime = 0; // Timer for rounding logic
      ...
      [](uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
        if (type == WStype_TEXT) {
          ...
          if (doc["type"] == "config") {
            ...
            if (doc["tick_rounding"].is<long>())
              tickRounding = doc["tick_rounding"];
          }
...
void loop() {
  ...
  if (deltaT >= 0.02f) { // 50Hz Control Loop
    ...
    if (millis() - lastTelemetryTime > 200) {
      ...
      enc["l"] = -cL; // NEGATE for App
      enc["r"] = -cR; // NEGATE for App
      ...
      Serial.printf("L_Ticks: %ld | R_Ticks: %ld | vL: %.2f | vR: %.2f%s\n", cL,
                    cR, vL_meas, vR_meas, wasStraight ? " [VA: ACTIVE]" : "");
    }

    // 6. ENCODER ROUNDING (Snap-to-Grid)
    // Only if feature is enabled and robot has been stopped for a while
    bool isStationary = (fabs(targetL) < 0.01f && fabs(targetR) < 0.01f &&
                         fabs(vL_meas) < 0.05f && fabs(vR_meas) < 0.05f);

    if (!isStationary) {
      lastStopSettlingTime = millis();
    } else if (tickRounding > 0 && (millis() - lastStopSettlingTime > 1000)) {
      // Check if we actually need to round (avoid repeating)
      long roundedL =
          (leftTicks >= 0)
              ? ((leftTicks + tickRounding / 2) / tickRounding) * tickRounding
              : ((leftTicks - tickRounding / 2) / tickRounding) * tickRounding;
      long roundedR =
          (rightTicks >= 0)
              ? ((rightTicks + tickRounding / 2) / tickRounding) * tickRounding
              : ((rightTicks - tickRounding / 2) / tickRounding) * tickRounding;

      if (leftTicks != roundedL || rightTicks != roundedR) {
        Serial.printf("Rounding Encoders: L(%ld -> %ld), R(%ld -> %ld)\n",
                      leftTicks, roundedL, rightTicks, roundedR);
        ...
        // Update baselines to prevent velocity jumps or straight-line kicks
        lastTicksL = roundedL;
        lastTicksR = roundedR;
        straightBaseL = roundedL;
        straightBaseR = roundedR;

        // Reset settling timer to avoid thrashing if noise remains
        lastStopSettlingTime = millis();
      }
    }
  }
